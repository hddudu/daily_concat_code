...
public class SingleTonForEHan2Synchronized {
	
	//加final的对象可以实例化吗? 不可以实例化
//	private static final SingleTonForEHan singleTon = null;//
	private static SingleTonForEHan2Synchronized singleTon = null;//
	
	/**
	 * 私有化构造器
	 */
	private SingleTonForEHan2Synchronized() {
	 ...
	 * @return
	 */
	public static synchronized SingleTonForEHan2Synchronized getInstance() {
		if(singleTon == null) {
...
//			singleTon = new SingleTonForEHan();//final修饰无法实例化
			singleTon = new SingleTonForEHan2Synchronized();//所以延迟初始化需要去掉final
		}
		return singleTon;
	}
